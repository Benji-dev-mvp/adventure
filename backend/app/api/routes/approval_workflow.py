...
async def get_pending_approvals(
    ...
    pending = [a for a in approval_queue if a["status"] == ApprovalStatus.PENDING]
...
async def get_approval(approval_id: int, current_user: User = Depends(get_current_user)):
        ...
        if item["id"] == approval_id:
            ...
            if response.edited_content and response.status == ApprovalStatus.NEEDS_REVISION:
                learning_insights = await _analyze_edits(
                    item["original_content"], response.edited_content, response.feedback
                )
...
async def get_approval_stats(current_user: User = Depends(get_current_user)):
...
async def fast_approve(approval_id: int, current_user: User = Depends(get_current_user)):
...
async def _analyze_edits(
    original: Dict[str, Any], edited: Dict[str, Any], feedback: Optional[str]
) -> Dict[str, Any]:
    """
    ...
    for key in edited:
        if key not in original:
            changes[key] = {"edited": edited[key], "type": "addition"}
