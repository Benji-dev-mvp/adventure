...
@router.post("/restore")
@require_role(UserRole.ADMIN)
async def restore_backup(backup_key: str, current_user: User = Depends(get_current_user)):
...
@require_role(UserRole.ADMIN)
async def cleanup_old_backups(days: int = 90, current_user: User = Depends(get_current_user)):
