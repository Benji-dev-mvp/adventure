...
async def generate_battle_card(
    ...
    talking_points = await _generate_talking_points(
        competitor_name, features, review_data
    )
    
    # Generate trap questions
    trap_questions = await _generate_trap_questions(
        competitor_name, features
    )
    
    # Identify vulnerabilities
    vulnerabilities = await _identify_vulnerabilities(
        competitor_name, review_data, features
    )
...
async def _generate_trap_questions(
    competitor: str,
    features: List[CompetitorFeature]
) -> List[str]:
...
async def _identify_vulnerabilities(
    ...
    if missing_features:
        vulnerabilities.append(
            f"Missing {len(missing_features)} key features we have"
        )
