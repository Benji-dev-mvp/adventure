...
def calculate_spam_score(subject: str, content: str) -> SpamCheckResponse:
    ...
    high_risk_words = {
        'free money': 25, 'click here': 20, 'buy now': 20, 'limited time': 15,
        'act now': 15, 'urgent': 12, 'winner': 15, 'congratulations': 12,
        '$$$': 20, 'cash': 15, 'prize': 15, 'guarantee': 12
    }
    
    medium_risk_words = {
        'free': 8, 'discount': 6, 'deal': 5, 'offer': 5,
        'save': 5, 'trial': 4, 'bonus': 6, 'gift': 6
    }
...
def calculate_optimal_send_times(
    ...
    if not timezones:
        timezone_dist = [
            TimezoneDistribution(zone="EST", percentage=42.0, lead_count=int(lead_count * 0.42), recommended_send_time="10:00 AM EST"),
            TimezoneDistribution(zone="CST", percentage=28.0, lead_count=int(lead_count * 0.28), recommended_send_time="9:00 AM CST"),
            TimezoneDistribution(zone="MST", percentage=15.0, lead_count=int(lead_count * 0.15), recommended_send_time="10:00 AM MST"),
            TimezoneDistribution(zone="PST", percentage=15.0, lead_count=int(lead_count * 0.15), recommended_send_time="8:00 AM PST"),
        ]
    else:
    ...
    worst_times = [
        ...
        {"day": "Any day", "time": "12:00-1:00 PM", "reason": "Lunch hour, emails get buried"},
...
def validate_campaign(
    ...
    if not any('{{' in step.get('content', '') for step in steps):
        recommendations.append("Add personalization tokens ({{firstName}}, {{company}}) to boost engagement")
...
async def optimize_send_time(
    request: SendTimeOptimizationRequest,
    current_user: User = Depends(get_current_user)
):
    """
