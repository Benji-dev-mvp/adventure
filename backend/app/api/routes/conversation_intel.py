...
async def analyze_transcript(
    ...
    all_insights = _compile_all_insights(
        pain_points, buying_signals, objections, budget_mentions, timeline_mentions
    )
    ...
    next_actions = _generate_next_actions(pain_points, objections, buying_signals, deal_score)
...
async def get_call_intelligence(call_id: str, current_user: User = Depends(get_current_user)):
...
def _parse_transcript(transcript: str) -> Dict[str, Any]:
    ...
    parsed = {"duration_minutes": 30, "exchanges": []}  # Extract from metadata
...
def _analyze_sentiment(data: Dict[str, Any]) -> SentimentAnalysis:
    ...
    return SentimentAnalysis(
        ...
        key_moments=[
            {
                "time": "2:30",
                "moment": "Prospect expressed frustration with current tool",
            },
