...
def playbook_to_response(playbook: Playbook) -> dict:
    ...
    return {
        ...
        "scheduled_days": (
            json.loads(playbook.scheduled_days) if playbook.scheduled_days else None
        ),
...
async def get_playbook(
    ...
    playbook = session.exec(
        select(Playbook).where(Playbook.id == playbook_id).where(Playbook.tenant_id == tenant_id)
    ).first()
...
async def delete_playbook(
    ...
    runs = session.exec(select(PlaybookRun).where(PlaybookRun.playbook_id == playbook_id)).all()
    for run in runs:
        session.delete(run)
...
async def execute_playbook_run(playbook_id: int, run_id: int, tenant_id: str, session: Session):
    """Background task to execute a playbook run"""
    import asyncio
    import random

    # Get the run
    run = session.exec(select(PlaybookRun).where(PlaybookRun.id == run_id)).first()
    ...
    try:
        ...
        playbook = session.exec(select(Playbook).where(Playbook.id == playbook_id)).first()
...
async def run_playbook(
    ...
    background_tasks.add_task(execute_playbook_run, playbook_id, run.id, tenant_id, session)
...
async def get_playbooks_summary(
    ...
    playbooks = session.exec(select(Playbook).where(Playbook.tenant_id == tenant_id)).all()
