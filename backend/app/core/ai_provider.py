...
class LLMClient:
    ...
    async def chat(self, messages: List[Dict[str, str]], stream: bool = False) -> str | AsyncGenerator:
...
def simple_lead_score(lead: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if any(k in title for k in ["vp", "cto", "head", "director"]):
        score += 20; rationale.append("senior title")
    if "ai" in industry or "saas" in industry or "cloud" in industry:
        score += 10; rationale.append("aligned industry")
    ...
    return {"score": score, "tier": tier, "rationale": ", ".join(rationale) or "baseline"}


def draft_email(lead: Dict[str, Any], prompt: str, tone: str = "professional", length: str = "medium") -> Dict[str, str]:
    ...
    try:
        if llm_client.provider != AIProvider.MOCK:
            import asyncio
            system_msg = f"You are an expert B2B sales email writer. Write {length} emails in a {tone} tone."
...
async def generate_campaign_content(
    ...
    user_prompt = f"Create {campaign_type} content for this audience: {target_audience}. Tone: {tone}"
