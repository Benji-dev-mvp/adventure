...
class GDPRCompliance:
    ...
    def export_user_data(user_id: int) -> Dict[str, Any]:
        ...
        with get_session() as session:
            ...
            # Leads owned by user
            leads = session.exec(select(Lead).where(Lead.owner_id == user_id)).all()
            data["leads"] = [lead.dict() for lead in leads]

            # Campaigns created by user
            campaigns = session.exec(select(Campaign).where(Campaign.created_by == user_id)).all()
            data["campaigns"] = [campaign.dict() for campaign in campaigns]

            # Audit logs
            audit_logs = session.exec(select(AuditLog).where(AuditLog.user_id == user_id)).all()
            data["audit_logs"] = [
                {
                    "action": log.action,
...
class DataRetentionPolicy:
    ...
    def cleanup_expired_data(resource_type: str):
        ...
        with get_session() as session:
            ...
            if resource_type == "audit_logs":
                ...
                result = session.exec(delete(AuditLog).where(AuditLog.timestamp < cutoff_date))
    ...
    def get_retention_report() -> Dict[str, Any]:
        ...
        report = {"generated_at": datetime.now().isoformat(), "policies": {}}

        for (
            resource_type,
            retention_days,
        ) in DataRetentionPolicy.RETENTION_PERIODS.items():
            cutoff_date = datetime.now() - timedelta(days=retention_days)
