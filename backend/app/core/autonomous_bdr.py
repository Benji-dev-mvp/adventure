...
class AutonomousAIBDR:
    ...
    async def research_prospect(self, lead: Dict) -> Dict:
        ...
        try:
            response = await chat_with_history([
                {"role": "system", "content": "You are an AI sales research assistant. Provide concise, actionable insights."},
    ...
    async def write_initial_email(self, lead: Dict, research: Dict) -> Dict:
        ...
        try:
            response = await chat_with_history([
                {"role": "system", "content": "You are Ava, an AI BDR expert at writing high-converting cold emails. You write like a human, not a bot."},
                {"role": "user", "content": prompt}
            ])
            ...
            subject = subject_match.group(1).strip() if subject_match else "Quick question about your outreach"
    ...
    async def analyze_reply(self, reply_text: str, lead: Dict) -> Dict:
        ...
        try:
            response = await chat_with_history([
                {"role": "system", "content": "You are an AI that analyzes sales email replies with expert precision."},
                {"role": "user", "content": prompt}
            ])
    ...
    def _detect_sentiment(self, text: str) -> str:
        ...
        positive_keywords = ['interested', 'yes', 'sounds good', 'let\'s chat', 'tell me more']
        negative_keywords = ['not interested', 'no thanks', 'unsubscribe', 'stop emailing']
    ...
    def _detect_intent(self, text: str) -> str:
        ...
        if any(kw in text_lower for kw in ['interested', 'tell me more', 'let\'s talk', 'meeting', 'call']):
            return 'interested'
    ...
    def _detect_objection(self, text: str) -> Optional[ObjectionType]:
        ...
        objection_patterns = {
            ...
            ObjectionType.ALREADY_HAVE_SOLUTION: ['already have', 'already using', 'current solution'],
            ObjectionType.NEED_TO_THINK: ['think about it', 'consider', 'get back to you'],
            ObjectionType.WRONG_PERSON: ['not the right person', 'wrong person', 'someone else'],
    ...
    async def handle_objection(self, objection_type: ObjectionType, original_reply: str, lead: Dict) -> Dict:
        """
        ...
        try:
            response = await chat_with_history([
                {"role": "system", "content": "You are Ava, an AI BDR expert at handling objections with empathy and skill."},
                {"role": "user", "content": prompt}
            ])
    ...
    def _fallback_objection_response(self, objection_type: ObjectionType, lead: Dict) -> Dict:
        ...
        return {
            'objection_type': objection_type,
            'response': responses.get(objection_type, "I understand. Would you still be open to a brief conversation?"),
            'handled_at': datetime.utcnow().isoformat(),
            'stage': ConversationStage.OBJECTION_HANDLING
        }
    ...
    async def book_meeting(self, lead: Dict, prospect_availability: Optional[str] = None) -> Dict:
        ...
        return {
            'lead_id': lead.get('id'),
            'meeting_time': meeting_slot.isoformat() if isinstance(meeting_slot, datetime) else meeting_slot,
    ...
    def _generate_confirmation_email(self, lead: Dict, meeting_time) -> str:
        ...
        time_str = meeting_time.strftime('%A, %B %d at %I:%M %p') if isinstance(meeting_time, datetime) else meeting_time
