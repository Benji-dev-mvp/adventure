...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, channel: str = "all", user_id: Optional[int] = None
    ):
        ...
        logger.info(
            f"WebSocket connected to channel: {channel}, total: {len(self.active_connections[channel])}"
        )
...
async def handle_websocket_commands(websocket: WebSocket, data: dict, user: User):
    ...
    if command == "subscribe":
        ...
        await manager.send_personal_message(
            {"type": "error", "message": f"Unknown command: {command}"}, websocket
        )
