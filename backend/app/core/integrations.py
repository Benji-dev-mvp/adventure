    ...
    async def _sync_to_hubspot(self, lead_data: Dict) -> Dict:
        ...
        hubspot_data = {
            "properties": [
                ...
                {"property": "firstname", "value": lead_data.get('name', '').split()[0]},
                {"property": "lastname", "value": ' '.join(lead_data.get('name', '').split()[1:])},
    ...
    async def _sync_to_pipedrive(self, lead_data: Dict) -> Dict:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response.raise_for_status()
                return {"success": True, "crm_id": response.json().get('data', {}).get('id')}
            except httpx.HTTPError as e:
...
class IntegrationRegistry:
    ...
    def list_active(self) -> List[str]:
        ...
        return [
            name for name, config in self.integrations.items()
            if config.enabled
        ]
