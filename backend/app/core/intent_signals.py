...
class IntentSignalEngine:
    ...
    async def scan_funding_rounds(self, company_domain: str, company_name: str) -> List[IntentSignal]:
        """
    ...
    async def scan_tech_stack_changes(self, company_domain: str, company_name: str) -> List[IntentSignal]:
        """
    ...
    async def scan_leadership_changes(self, company_domain: str, company_name: str) -> List[IntentSignal]:
        """
    ...
    def calculate_company_intent_score(self, company_domain: str) -> Dict:
        ...
        return {
            ...
            'breakdown': {
                'job_postings': len([s for s in signals if s.signal_type == IntentSignalType.JOB_POSTING]),
                'funding': len([s for s in signals if s.signal_type == IntentSignalType.FUNDING_ROUND]),
                'tech_changes': len([s for s in signals if s.signal_type == IntentSignalType.TECH_STACK_CHANGE]),
                'leadership': len([s for s in signals if s.signal_type == IntentSignalType.LEADERSHIP_CHANGE])
            }
        }
