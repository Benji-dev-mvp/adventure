...
class FeatureRegistry:
    ...
    def search_features(self, tags: Optional[List[str]] = None, entity_type: Optional[str] = None) -> List[FeatureMetadata]:
        ...
        for entity_id in entity_ids:
            ...
            for feature_name in feature_names:
                ...
                if start_date or end_date:
                    values = [
                        fv for fv in values
                        if (not start_date or fv.timestamp >= start_date) and
                           (not end_date or fv.timestamp <= end_date)
                    ]
    ...
    def get_feature_stats(self, feature_name: str) -> Dict[str, Any]:
        ...
        for key, feature_values in self.offline_storage.items():
            if key.startswith(f"{feature_name}:"):
                values.extend([fv.value for fv in feature_values if isinstance(fv.value, (int, float))])
