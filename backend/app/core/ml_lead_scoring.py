...
class ProductionLeadScorer:
    ...
    def _extract_features(self, lead_data: Dict) -> np.ndarray:
        ...
        social_score = min(
            (
                ...
                + lead_data.get("twitter_followers", 0) / 1000
            )
            / 2,
            1.0,
        )
        ...
        tech_match = (
            len(set(tech_stack) & set(target_tech)) / max(len(target_tech), 1)
            if target_tech
            else 0.5
        )
    ...
    def train(self, training_data: List[Dict], labels: List[int]) -> Dict:
        ...
        if not XGBOOST_AVAILABLE:
            raise ImportError(
                "XGBoost not available. Install with: pip install xgboost scikit-learn"
            )
        ...
        logger.info(
            f"Model trained successfully. Accuracy: {metrics['accuracy']:.3f}, AUC: {metrics['auc_roc']:.3f}"
        )
...
def generate_synthetic_training_data(n_samples: int = 1000) -> tuple:
    ...
    for _ in range(n_samples):
        ...
        lead = {
            "company_size": (
                random.choice([10, 50, 100, 500, 1000, 5000])
                if converted
                else random.choice([5, 20, 50, 100])
            ),
            ...
            "tech_stack": random.sample(
                ["Salesforce", "HubSpot", "Marketo", "Outreach"], k=random.randint(0, 3)
            ),
