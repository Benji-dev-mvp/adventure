...
class MultiTierCache:
    ...
    def get_hot_keys(self, top_n: int = 10) -> List[tuple[str, int]]:
        ...
        """
        access_counts = {key: len(accesses) for key, accesses in self.access_patterns.items()}
    ...
    def predict_invalidation(self, key: str) -> Optional[datetime]:
        ...
        intervals = [
            (accesses[i] - accesses[i - 1]).total_seconds() for i in range(1, len(accesses))
        ]
...
class CacheWarmer:
    ...
    async def schedule_warming(
        self, query_func: Callable, schedule_time: datetime, keys: List[str]
    ) -> None:
        """
...
def cached(ttl: int = 300, key_prefix: str = "", skip_cache_if: Optional[Callable] = None):
    """
...
async def example_cached_query(lead_id: int) -> Dict[str, Any]:
    ...
    async def _fetch_lead(lead_id: int):
        ...
        return {"id": lead_id, "name": f"Lead {lead_id}", "score": 75}
