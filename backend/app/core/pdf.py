...
class PDFGenerator:
    ...
    def generate_campaign_report(campaign_data: Dict[str, Any]) -> BytesIO:
        ...
        metrics_data = [
            ...
            [
                "Open Rate",
                str(campaign_data.get("open_rate", 0)),
                f"{campaign_data.get('open_rate', 0)}%",
            ],
            [
                "Click Rate",
                str(campaign_data.get("click_rate", 0)),
                f"{campaign_data.get('click_rate', 0)}%",
            ],
            [
                "Response Rate",
                str(campaign_data.get("response_rate", 0)),
                f"{campaign_data.get('response_rate', 0)}%",
            ],
            [
                "Bounce Rate",
                str(campaign_data.get("bounce_rate", 0)),
                f"{campaign_data.get('bounce_rate', 0)}%",
            ],
        ]
    ...
    def generate_audit_log_report(logs: List[Dict[str, Any]]) -> BytesIO:
        ...
        # Title
        title = Paragraph(
            f"Audit Log Report - {datetime.now().strftime('%Y-%m-%d')}", styles["Title"]
        )
        elements.append(title)
        elements.append(Spacer(1, 0.3 * inch))
