...
class StreamProcessor:
    ...
    async def reduce(
        self,
        func: Callable[[Any, StreamEvent], Any],
        initial: Any
    ) -> AsyncIterator[Any]:
        """
...
class RealTimeAnalytics:
    ...
    async def session_window(
        ...
        async for event in stream:
            if last_event_time and (event.timestamp - last_event_time).total_seconds() > gap_seconds:
...
async def campaign_metrics_stream(campaign_id: str) -> AsyncIterator[Dict[str, Any]]:
    ...
    """
    from app.core.event_sourcing import event_store, EventType
