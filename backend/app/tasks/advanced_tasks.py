...
def enrich_uploaded_leads(upload_id: int):
    ...
    with get_session() as session:
        from app.models.schemas import Lead
        leads = session.exec(
            select(Lead).where(Lead.upload_id == upload_id)
        ).all()
...
def schedule_campaign_sends(campaign_id: int):
    ...
    with get_session() as session:
        ...
        for i, lead in enumerate(leads):
            ...
            task = send_campaign_email.apply_async(
                args=[campaign_id, lead.id],
                eta=eta
            )
...
def trigger_lead_actions():
    ...
    with get_session() as session:
        ...
        high_score_leads = session.exec(
            select(Lead).where(Lead.score >= 80)
        ).all()
