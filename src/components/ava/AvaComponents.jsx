...
import { Bot, Mail, TrendingUp, Sparkles, Zap, ThumbsUp, ThumbsDown, Clock, CheckCircle, AlertCircle, Send, Brain, Target, Loader } from 'lucide-react';
...
export const AvaChatInterface = () => {
  ...
  const [messages, setMessages] = useState([
    { sender: 'ava', text: '👋 Hi! I\'m Ava, your AI BDR. I\'ve analyzed your target accounts and found 247 high-intent prospects. Ready to launch a campaign?', time: '2 min ago' },
    { sender: 'user', text: 'Yes! What do you recommend?', time: '1 min ago' },
    { sender: 'ava', text: '🎯 I recommend a 5-step email sequence targeting VP of Sales at Series B companies. I found 89 prospects who just raised funding. Want me to set it up?', time: '30 sec ago' }
  ]);
      ...
      <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] ${msg.sender === 'ava' ? 'bg-purple-50 dark:bg-purple-900/20' : 'bg-blue-500 text-white'} rounded-lg p-3`}>
              <p className="text-sm">{msg.text}</p>
              <p className={`text-xs mt-1 ${msg.sender === 'ava' ? 'text-gray-600 dark:text-gray-400' : 'text-blue-100'}`}>{msg.time}</p>
            </div>
          </div>
        ))}
        ...
        <div className="space-y-2 pt-4">
          <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">💡 AI Recommendations (click to use):</p>
          <div className="flex flex-wrap gap-2">
            <Button 
              size="sm" 
              variant="outline" 
              className="text-xs hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors"
              onClick={() => handleQuickAction('Show me my campaign performance from the last 30 days')}
            >
            ...
            <Button 
              ...
              variant="outline" 
              className="text-xs hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors"
              onClick={() => handleQuickAction('Find me 50 more high-intent prospects in my target ICP')}
            >
              🎯 Find more prospects
            </Button>
            <Button 
              ...
              variant="outline" 
              className="text-xs hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors"
              onClick={() => handleQuickAction('Write a personalized email for my top prospect with their recent funding news')}
            >
            ...
            <Button 
              ...
              variant="outline" 
              className="text-xs hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors"
              onClick={() => handleQuickAction('Create a 5-step outbound sequence for Series B SaaS companies')}
            >
              🚀 Create sequence
            </Button>
            <Button 
...
export const EmailDeliverabilityDashboard = () => {
  ...
  const [mailboxes] = useState([
    { email: 'john@company.com', status: 'healthy', warmupProgress: 87, dailyLimit: 50, sent: 42, health: 94 },
    { email: 'sarah@company.com', status: 'warming', warmupProgress: 34, dailyLimit: 20, sent: 15, health: 78 },
    { email: 'mike@company.com', status: 'healthy', warmupProgress: 100, dailyLimit: 80, sent: 63, health: 98 }
  ]);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Ava manages your mailbox health automatically</p>
      </CardHeader>
      <CardContent>
            ...
            <div key={idx} className="p-4 border rounded-lg">
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <Badge variant={box.status === 'healthy' ? 'success' : 'warning'} className="text-xs mt-1">
...
export const DataMinerDashboard = () => {
  ...
  const [dataSourcesActivity] = useState([
    ...
    { source: 'Press Releases', icon: '📰', insights: 156, status: 'active', lastUpdate: '15 min ago' },
    { source: 'Job Postings', icon: '👔', insights: 523, status: 'active', lastUpdate: '8 min ago' },
    { source: 'Company News', icon: '📢', insights: 234, status: 'active', lastUpdate: '12 min ago' }
  ]);

  const handleSourceClick = (source) => {
    showToast(`Viewing ${source.insights} insights from ${source.source}...`, 'info');
    setTimeout(() => {
      showToast(`Found ${Math.floor(Math.random() * 50)} high-intent signals in ${source.source}!`, 'success');
    }, 1000);
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Ava is actively mining data from 6 sources</p>
      </CardHeader>
      <CardContent>
        ...
        <div className="space-y-2">
          {dataSourcesActivity.map((source, idx) => (
            ...
            >
              ...
              <div className="text-right">
                <Badge variant="success" className="text-xs mb-1">{source.status}</Badge>
                <p className="text-xs text-gray-600 dark:text-gray-400">{source.insights} insights</p>
              </div>
            </div>
          ))}
        </div>
...
export const PersonalizationWaterfallViewer = () => {
  const [prospect] = useState({
    ...
    company: 'Acme Corp',
    waterfall: [
      { level: '1st Priority', type: 'Recent Job Change', available: true, data: 'Started VP of Sales 2 weeks ago' },
      { level: '2nd Priority', type: 'Funding Round', available: true, data: 'Series B - $25M raised 3 months ago' },
      { level: '3rd Priority', type: 'Twitter Activity', available: true, data: 'Posted about sales automation' },
      { level: '4th Priority', type: 'Hiring Activity', available: false, data: null },
      { level: '5th Priority', type: 'Company News', available: true, data: 'Opened new office in Austin' },
      { level: 'Fallback', type: 'Generic Template', available: true, data: 'Industry-based template' }
    ]
  });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">How Ava chooses the best personalization</p>
      </CardHeader>
      <CardContent>
        <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <p className="text-sm font-semibold">{prospect.name} @ {prospect.company}</p>
          <Badge variant="success" className="text-xs mt-1">✓ Best Personalization Found</Badge>
        </div>
        ...
        <div className="mt-4 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
          <p className="text-xs font-semibold text-purple-700 dark:text-purple-300">✨ Ava's Choice</p>
          <p className="text-sm mt-1">"Hi John, congrats on the VP of Sales role! I noticed Acme just raised $25M..."</p>
        </div>
      </CardContent>
    </Card>
  );
...
export const SentimentAnalysisDashboard = () => {
  const [responses] = useState([
    { from: 'john@acme.com', subject: 'Re: Interested', sentiment: 'positive', score: 92, text: 'This looks great! Can we schedule a demo?', time: '5 min ago' },
    { from: 'sarah@tech.co', subject: 'Re: Not now', sentiment: 'negative', score: 15, text: 'Not interested at this time', time: '12 min ago' },
    { from: 'mike@startup.io', subject: 'Re: Tell me more', sentiment: 'positive', score: 78, text: 'Interesting! Send me more info', time: '18 min ago' },
    { from: 'lisa@corp.com', subject: 'Re: Maybe later', sentiment: 'neutral', score: 45, text: 'Let\'s reconnect next quarter', time: '25 min ago' }
  ]);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Ava automatically detects response sentiment</p>
      </CardHeader>
      <CardContent>
        ...
        <div className="space-y-2">
          {responses.map((response, idx) => (
            <div key={idx} className={`p-3 rounded-lg border-l-4 ${
              response.sentiment === 'positive' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' :
              response.sentiment === 'negative' ? 'border-red-500 bg-red-50 dark:bg-red-900/20' :
              'border-gray-300 bg-gray-50 dark:bg-gray-800/50'
            }`}>
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="text-right">
                  <Badge variant={
                    ...
                    response.sentiment === 'negative' ? 'danger' : 'secondary'
                  } className="text-xs">
...
export const LeadQualificationPipeline = () => {
  const [qualifiedLeads] = useState([
    { name: 'John Doe', company: 'Acme Corp', score: 94, status: 'hot', reason: 'Requested demo, opened 3 emails', handedOff: false },
    { name: 'Sarah Smith', company: 'Tech Co', score: 88, status: 'warm', reason: 'Positive reply, high engagement', handedOff: false },
    { name: 'Mike Johnson', company: 'Startup Inc', score: 76, status: 'warm', reason: 'Downloaded whitepaper, visited pricing', handedOff: true }
  ]);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Hot leads Ava has handed over to your sales team</p>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
          {qualifiedLeads.map((lead, idx) => (
            <div key={idx} className="p-4 border rounded-lg bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/10 dark:to-orange-900/10">
              <div className="flex justify-between items-start mb-2">
                <div>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Ava is learning and optimizing continuously</p>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="text-center p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={performanceData}>
            ...
            <Tooltip />
            <Legend />
            <Line type="monotone" dataKey="responseRate" stroke="#8b5cf6" name="Response Rate %" strokeWidth={2} />
            <Line type="monotone" dataKey="meetingsBooked" stroke="#10b981" name="Meetings Booked" strokeWidth={2} />
          </LineChart>
        </ResponsiveContainer>
        ...
        <div className="mt-4 p-3 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-lg">
          ...
          <p className="text-xs text-gray-600 mt-1">Adjusted email tone to be more conversational based on positive feedback patterns. Expected +5% boost in response rate.</p>
        </div>
      </CardContent>
    </Card>
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Provide coaching to improve Ava's performance</p>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button variant="outline" size="sm">Shorter Emails</Button>
          ...
          <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <p className="text-xs font-semibold text-blue-700 dark:text-blue-300">Recent Training</p>
...
export const B2BDatabaseSearch = () => {
  const [results] = useState([
    { name: 'John Doe', title: 'VP of Sales', company: 'Acme Corp', employees: '501-1000', verified: true },
    { name: 'Sarah Smith', title: 'Director of Marketing', company: 'Tech Co', employees: '201-500', verified: true },
    { name: 'Mike Johnson', title: 'CEO', company: 'Startup Inc', employees: '11-50', verified: true }
  ]);
              ...
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {contact.verified && <Badge variant="success" className="text-xs">✓ Verified</Badge>}
                  </div>
                  <p className="text-xs text-gray-600">{contact.title}</p>
                  <p className="text-xs text-gray-600">{contact.company} • {contact.employees} employees</p>
                </div>
