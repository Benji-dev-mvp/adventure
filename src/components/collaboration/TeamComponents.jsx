...
import { MessageSquare, Mail, Users, Calendar, Activity, Briefcase, Video, MessageCircle } from 'lucide-react';

export const SharedInbox = () => {
  const [emails, setEmails] = useState([
    { id: 1, from: 'john@acme.com', subject: 'Re: Pricing question', assignedTo: 'Sarah', status: 'open', replies: 2 },
    { id: 2, from: 'jane@techco.com', subject: 'Demo request', assignedTo: 'Unassigned', status: 'new', replies: 0 },
    { id: 3, from: 'bob@startup.io', subject: 'Follow-up from call', assignedTo: 'Mike', status: 'in-progress', replies: 5 }
  ]);
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-2">
          {emails.map(email => (
            <div key={email.id} className="p-3 border rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 cursor-pointer">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h4 className="font-semibold text-sm">{email.from}</h4>
                  <p className="text-xs text-gray-600 mt-1">{email.subject}</p>
                </div>
                <Badge variant={email.status === 'new' ? 'error' : email.status === 'open' ? 'warning' : 'secondary'}>
                  {email.status}
                </Badge>
              </div>
...
export const LeadNotesComments = () => {
  const [notes, setNotes] = useState([
    { user: 'Sarah', text: '@Mike can you follow up on the pricing question?', time: '2 min ago', mentions: ['Mike'] },
    { user: 'Mike', text: 'Had a great call today. They want to move forward!', time: '1 hour ago', mentions: [] },
    { user: 'Lisa', text: '@Sarah @Mike Let\'s schedule a handoff meeting', time: '3 hours ago', mentions: ['Sarah', 'Mike'] }
  ]);
...
export const LeadHandoffWorkflow = () => {
  const [handoffs, setHandoffs] = useState([
    { lead: 'John Doe - Acme Corp', from: 'SDR: Sarah', to: 'AE: Mike', status: 'pending', score: 85 },
    { lead: 'Jane Smith - TechCo', from: 'SDR: Lisa', to: 'AE: Sarah', status: 'accepted', score: 92 },
    { lead: 'Bob Johnson - Startup', from: 'SDR: Mike', to: 'AE: Lisa', status: 'scheduled', score: 78 }
  ]);
          ...
          {handoffs.map((handoff, idx) => (
            <div key={idx} className="p-3 border rounded-lg">
              <div className="flex justify-between items-start mb-2">
                ...
                <Badge variant={
                  handoff.status === 'pending' ? 'warning' :
                  handoff.status === 'accepted' ? 'success' : 'secondary'
                }>
                  {handoff.status}
                </Badge>
              </div>
      ...
      <CardContent>
        <div className="space-y-2">
          {events.map((event, idx) => (
            <div key={idx} className="p-3 border-l-4 rounded bg-gray-50 dark:bg-white/5"
                 style={{ borderLeftColor: event.type === 'demo' ? '#3b82f6' : event.type === 'call' ? '#10b981' : '#6b7280' }}>
              ...
              <div className="flex gap-1 flex-wrap">
                {event.attendees.map(att => (
                  <Badge key={att} variant="secondary" className="text-xs">{att}</Badge>
                ))}
              </div>
            </div>
          ))}
...
export const TeamActivityFeed = () => {
  const activities = [
    { user: 'Sarah', action: 'closed a deal', target: 'Acme Corp - $45K', time: '5 min ago', icon: '🎉' },
    { user: 'Mike', action: 'scheduled demo with', target: 'TechCo', time: '15 min ago', icon: '📅' },
    { user: 'Lisa', action: 'sent proposal to', target: 'StartupXYZ', time: '1 hour ago', icon: '📄' },
    { user: 'Sarah', action: 'added 50 leads from', target: 'LinkedIn', time: '2 hours ago', icon: '➕' }
  ];
...
export const DealRooms = () => {
  const [rooms, setRooms] = useState([
    { name: 'Acme Corp Deal', members: 4, files: 12, messages: 34, stage: 'Proposal', value: '$45K' },
    { name: 'TechCo Expansion', members: 3, files: 8, messages: 23, stage: 'Negotiation', value: '$78K' },
            ...
            <div key={idx} className="p-3 border rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 cursor-pointer">
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <Badge variant="secondary" className="text-xs mt-1">{room.stage}</Badge>
                </div>
...
export const VideoCallIntegration = () => {
  const [recordings, setRecordings] = useState([
    { title: 'Demo - Acme Corp', duration: '45:32', date: 'Dec 20', participants: 3, transcribed: true },
    { title: 'Discovery Call - TechCo', duration: '32:15', date: 'Dec 19', participants: 2, transcribed: true },
    { title: 'Follow-up - StartupXYZ', duration: '28:43', date: 'Dec 18', participants: 4, transcribed: false }
  ]);
            ...
            <div key={idx} className="p-3 border rounded-lg">
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-1">{rec.date} • {rec.participants} participants</p>
                </div>
                <Badge variant="secondary">{rec.duration}</Badge>
              </div>
              <div className="flex gap-2">
                <Button size="sm" variant="outline">Play</Button>
                {rec.transcribed && <Button size="sm" variant="outline">View Transcript</Button>}
              </div>
            </div>
          ))}
        </div>
