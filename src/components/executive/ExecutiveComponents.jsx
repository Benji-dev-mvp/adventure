...
import { TrendingUp, Activity, DollarSign, Users, Target, AlertTriangle, CheckCircle, BarChart3, Zap } from 'lucide-react';
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant={timeframe === 'week' ? 'default' : 'outline'} onClick={() => setTimeframe('week')}>Week</Button>
            <Button size="sm" variant={timeframe === 'month' ? 'default' : 'outline'} onClick={() => setTimeframe('month')}>Month</Button>
            <Button size="sm" variant={timeframe === 'quarter' ? 'default' : 'outline'} onClick={() => setTimeframe('quarter')}>Quarter</Button>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          ...
          <div className="p-4 bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl text-white">
            ...
            <p className="text-3xl font-bold mt-1">${(metrics.pipeline.value / 1000000).toFixed(1)}M</p>
            <div className="flex items-center gap-1 mt-2 text-sm">
              <TrendingUp size={14} />
          ...
          <div className="p-4 bg-gradient-to-br from-orange-500 to-red-500 rounded-xl text-white">
            ...
            <Badge variant="success" className="mt-2 text-xs">Excellent</Badge>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={revenueData}>
              ...
              <Tooltip />
              <Legend />
              <Area type="monotone" dataKey="arr" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.3} name="ARR ($K)" />
              <Area type="monotone" dataKey="mrr" stroke="#10b981" fill="#10b981" fillOpacity={0.3} name="MRR ($K)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
...
export const PipelineHealthScore = () => {
  ...
  const [healthFactors] = useState([
    { factor: 'Pipeline Coverage', score: 92, status: 'excellent', description: '4.2x coverage of quarterly quota' },
    { factor: 'Deal Velocity', score: 85, status: 'good', description: '49 days avg cycle time (target: 45)' },
    { factor: 'Stage Distribution', score: 88, status: 'excellent', description: 'Balanced across all stages' },
    { factor: 'Win Rate Trend', score: 90, status: 'excellent', description: '24% win rate, +3% vs last quarter' },
    { factor: 'Deal Age', score: 78, status: 'warning', description: '12 deals over 90 days old' },
    { factor: 'Activity Level', score: 95, status: 'excellent', description: '847 touches this week' }
  ]);
  ...
  const getStatusBadge = (status) => {
    if (status === 'excellent') return <Badge variant="success" className="text-xs">Excellent</Badge>;
    if (status === 'good') return <Badge variant="primary" className="text-xs">Good</Badge>;
    if (status === 'warning') return <Badge variant="warning" className="text-xs">Needs Attention</Badge>;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Real-time pipeline quality assessment</p>
      </CardHeader>
      <CardContent>
        {/* Overall Score */}
...
export const ForecastAccuracyTracker = () => {
  ...
  const avgAccuracy = Math.round(forecastData.reduce((sum, q) => sum + q.accuracy, 0) / forecastData.length);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Predicted vs actual revenue comparison</p>
      </CardHeader>
      <CardContent>
        {/* Accuracy Score */}
        <div className="mb-4 p-4 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl text-white text-center">
          ...
          <Badge variant="success" className="mt-2">Industry Leading</Badge>
        </div>
            ...
            <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  ...
                  <p className="text-sm">${quarter.forecasted}K vs ${quarter.actual}K</p>
                </div>
...
export const WhatIfScenarioPlanner = () => {
  ...
  const calculateImpact = () => {
    ...
    const baseRevenue = (baseMetrics.winRate / 100) * baseMetrics.avgDealSize * baseMetrics.monthlyDeals * 12;
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm font-semibold mb-2 flex items-center justify-between">
                ...
                <Badge variant="secondary" className="text-xs">{scenario.winRateIncrease}%</Badge>
              </label>
              <input 
                type="range" 
                min="0" 
            ...
            <div>
              <label className="text-sm font-semibold mb-2 flex items-center justify-between">
                ...
                <Badge variant="secondary" className="text-xs">{scenario.dealSizeIncrease}%</Badge>
              </label>
              <input 
                type="range" 
                min="0" 
            ...
            <div>
              <label className="text-sm font-semibold mb-2 flex items-center justify-between">
                ...
                <Badge variant="secondary" className="text-xs">{scenario.velocityImprovement}%</Badge>
              </label>
              <input 
                type="range" 
                min="0" 
            ...
            <Button 
              className="w-full"
              onClick={() => setScenario({ winRateIncrease: 0, dealSizeIncrease: 0, velocityImprovement: 0 })}
            >
              Reset
            </Button>
          </div>
          ...
          <div>
            <div className="p-6 bg-gradient-to-r from-green-500 to-emerald-500 rounded-xl text-white mb-4">
              ...
              <p className="text-5xl font-bold mt-2">${(impact.revenueIncrease / 1000).toFixed(0)}K</p>
            ...
            <div className="space-y-3">
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <p className="text-xs text-green-600 mt-1">+{(impact.newWinRate - baseMetrics.winRate).toFixed(1)}% improvement</p>
              </div>
              <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                <p className="text-xs text-gray-600 mb-1">New Avg Deal Size</p>
                <p className="text-3xl font-bold text-purple-600">${(impact.newDealSize / 1000).toFixed(0)}K</p>
                <p className="text-xs text-green-600 mt-1">+${(impact.newDealSize - baseMetrics.avgDealSize).toLocaleString()}</p>
              </div>
              <div className="p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                ...
                <p className="text-xs text-green-600 mt-1">-{baseMetrics.avgCycleTime - impact.newCycleTime} days faster</p>
              </div>
            </div>
          </div>
        </div>
