...
const CustomerShowcase = () => {
  ...
  const testimonials = [
    {
      quote: "Artisan's Ava has transformed our outbound motion. We've 3x'd our pipeline in 6 months while cutting SDR costs by 60%. The AI personalization is remarkably accurate.",
      author: "Sarah Chen",
      title: "VP of Sales",
    ...
    {
      quote: "The governance controls give our security team peace of mind while empowering our reps to move fast. SOC 2 compliance out of the box was a game-changer.",
    ...
    {
      quote: "We went from 50 qualified meetings per month to 180. Ava doesn't just send emails—it orchestrates intelligent, multi-channel campaigns that actually convert.",
      author: "Emily Watson",
  ...
  return (
    <div id="customers" className="w-full py-20 bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-slate-600 dark:text-slate-300 max-w-3xl mx-auto">
            From fast-growing startups to Fortune 500 enterprises, teams trust Artisan to drive predictable revenue growth
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
          {impactMetrics.map((metric, idx) => {
            ...
            return (
              <Card key={idx} className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-center hover:shadow-xl transition-shadow">
                ...
                {customers.map((customer, idx) => (
                  ...
                  >
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                      <div className={`w-16 h-16 rounded-xl bg-gradient-to-br ${customer.color} flex items-center justify-center text-white font-bold text-xl mb-2 shadow-lg`}>
                        {customer.logo}
                      </div>
                      <div className="text-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <div className="text-xs font-semibold text-slate-900 dark:text-white">{customer.name}</div>
                        <div className="text-xs text-slate-500 dark:text-slate-400">{customer.industry}</div>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="flex items-center justify-between flex-wrap gap-6 mb-6">
                ...
                <div className="flex gap-4">
                  {Object.entries(testimonials[activeTestimonial].metrics).map(([key, value]) => (
                    <div key={key} className="text-center px-4 py-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                      <div className="text-lg font-bold text-purple-600 dark:text-purple-400">{value}</div>
                      <div className="text-xs text-slate-600 dark:text-slate-400 capitalize">{key}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-12">
          {useCases.map((useCase, idx) => {
            ...
            return (
              <Card key={idx} className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:shadow-xl transition-all group">
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-semibold text-green-600 dark:text-green-400">{useCase.results}</span>
                  </div>
                </CardContent>
              </Card>
            );
        ...
        <Card className="bg-gradient-to-r from-blue-600 to-purple-600 border-0 text-white">
          <CardContent className="p-8">
            <div className="flex items-center justify-between flex-wrap gap-6">
              <div>
                <h3 className="text-2xl font-bold mb-2">See How TechCorp 3x'd Pipeline in 6 Months</h3>
                <p className="text-blue-100">Download the full case study with implementation details and ROI breakdown</p>
              </div>
