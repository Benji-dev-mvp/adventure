...
const InteractiveArchitectureFlow = () => {
  ...
  const steps = [
    ...
    },
    {
      ...
      data: {
        policies: ["DLP scan: ✓ Passed", "Consent check: ✓ Opt-in verified", "Send limits: ✓ Within quota"],
    ...
    },
    {
      ...
      data: {
        ...
        aiLearning: ["Subject line A performed +23% better", "Tuesday 10am optimal", "Case study CTA effective"],
  ...
  return (
    <section id="how-it-works" className="py-20 bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            See exactly how each component works together—from database query to closed deal. This is the architecture your security team needs to see.
          </p>
        </div>
                  ...
                  >
                    <div 
                      className={`w-32 h-32 rounded-xl border-2 flex flex-col items-center justify-center p-3 transition-all ${
                        isActive 
                          ? 'bg-gradient-to-br ' + step.color + ' border-white shadow-2xl shadow-purple-500/50'
                          : 'bg-slate-800 border-slate-600 hover:border-slate-500'
                      }`}
                    >
                      <Icon className={`w-8 h-8 mb-2 ${isActive ? 'text-white' : 'text-slate-400'}`} />
                  ...
                  {idx < steps.length - 1 && (
                    <div className="flex items-center justify-center">
                      <ArrowRight className={`w-6 h-6 transition-colors ${
                        activeStep > idx ? 'text-green-400' : activeStep === idx ? 'text-white animate-pulse' : 'text-slate-600'
                      }`} />
                    </div>
                  )}
                </React.Fragment>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-4">
                  ...
                  {currentStep.screenshot.type === 'search' && (
                    <div className="space-y-2">
                      {currentStep.screenshot.results.map((result, idx) => (
                        <div key={idx} className="bg-slate-800 border border-slate-600 rounded-lg p-3 hover:border-blue-500 transition-colors">
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="font-semibold text-white">{result.name}</div>
                              <div className="text-sm text-gray-400">{result.title} at {result.company}</div>
                            </div>
                  ...
                  {currentStep.screenshot.type === 'analysis' && (
                    <div className="space-y-2">
                      {currentStep.screenshot.insights.map((insight, idx) => (
                        <div key={idx} className="flex items-start gap-2 p-3 bg-slate-800 border border-slate-600 rounded-lg">
                  ...
                  {currentStep.screenshot.type === 'content' && (
                    <div className="bg-slate-800 border border-slate-600 rounded-lg p-4">
                      <div className="mb-2">
                        ...
                        <div className="text-white font-semibold">{currentStep.screenshot.email.subject}</div>
                      </div>
                      <div className="text-xs text-gray-400 mb-1">Body:</div>
                      <div className="text-gray-300 text-sm whitespace-pre-line">{currentStep.screenshot.email.body}</div>
                  ...
                  {currentStep.screenshot.type === 'engagement' && (
                    <div className="space-y-2">
                      {currentStep.screenshot.activity.map((act, idx) => (
                        <div key={idx} className="flex items-start gap-3 p-3 bg-slate-800 border border-slate-600 rounded-lg">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-gray-400 text-xs">
                              {Object.entries(act).filter(([key]) => !['time', 'event'].includes(key)).map(([key, value]) => (
                  ...
                  {currentStep.screenshot.type === 'qualification' && (
                    <div className="bg-slate-800 border border-slate-600 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-4">
                        <h5 className="text-white font-bold">{currentStep.screenshot.profile.name}</h5>
                      ...
                      <div className="mb-3">
                        ...
                        <div className="text-white font-semibold">{currentStep.screenshot.profile.stage}</div>
                      </div>
                      <div className="mb-3">
                  ...
                  {currentStep.screenshot.type === 'analytics' && (
                    <div className="grid grid-cols-2 gap-3">
                      {currentStep.screenshot.insights.map((insight, idx) => (
                        <div key={idx} className="bg-slate-800 border border-slate-600 rounded-lg p-4">
                          ...
                          <div className="text-xs text-green-400 font-semibold">{insight.benchmark}</div>
                        </div>
                      ))}
                    </div>
                  )}
