...
export const TeamLeaderboard = () => {
  ...
  const [leaderboard] = useState([
    { rank: 1, name: 'Sarah Johnson', avatar: '👩', points: 2847, metrics: { emails: 342, meetings: 28, deals: 5 }, streak: 12, badge: '🏆' },
    { rank: 2, name: 'Mike Chen', avatar: '👨', points: 2654, metrics: { emails: 298, meetings: 24, deals: 4 }, streak: 8, badge: '🥈' },
    { rank: 3, name: 'Emily Davis', avatar: '👩', points: 2431, metrics: { emails: 276, meetings: 22, deals: 4 }, streak: 6, badge: '🥉' },
    { rank: 4, name: 'John Smith', avatar: '👨', points: 2198, metrics: { emails: 254, meetings: 19, deals: 3 }, streak: 5, badge: '' },
    { rank: 5, name: 'Lisa Wong', avatar: '👩', points: 2087, metrics: { emails: 241, meetings: 17, deals: 3 }, streak: 4, badge: '' }
  ]);
                ...
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Badge variant="secondary" className="text-xs">{person.points} pts</Badge>
                      {person.streak > 0 && (
...
export const AchievementBadges = () => {
  const [achievements] = useState([
    { id: 1, name: 'First Blood', icon: '🎯', description: 'Booked your first meeting', earned: true, date: 'Dec 10' },
    { id: 2, name: 'Speed Demon', icon: '⚡', description: 'Sent 100 emails in one day', earned: true, date: 'Dec 15' },
    { id: 3, name: 'Closer', icon: '💰', description: 'Closed 5 deals', earned: true, date: 'Dec 20' },
    { id: 4, name: 'Streak Master', icon: '🔥', description: '10-day activity streak', earned: true, date: 'Dec 22' },
    { id: 5, name: 'Social Butterfly', icon: '🦋', description: 'Connect with 50 prospects on LinkedIn', earned: false, progress: 38 },
    { id: 6, name: 'Unicorn Hunter', icon: '🦄', description: 'Land a $100K+ deal', earned: false, progress: 0 }
  ]);
      ...
      <CardContent>
        <div className="space-y-4">
          {competitions.map((comp) => (
            <div key={comp.id} className="p-4 rounded-lg border-2 border-purple-300 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="font-bold text-lg">{comp.name}</h3>
                  <p className="text-sm text-gray-600">{comp.type} • {comp.metric}</p>
                  ...
                  {comp.teams.map((team, idx) => (
                    <div 
                      ...
                      className={`flex items-center justify-between p-3 rounded ${
                        team.winning ? 'bg-green-100 dark:bg-green-900/30' : 'bg-gray-100 dark:bg-gray-800/50'
                      }`}
                    >
                      <div className="flex items-center gap-2">
                      ...
                      <div className="text-right">
                        ...
                        {team.winning && <Badge variant="success" className="text-xs">LEADING</Badge>}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="mb-4 p-6 bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl text-white">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="text-right">
              ...
              <Badge variant="warning" className="text-xs">#{userPoints.rank} Rank</Badge>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {userPoints.history.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between p-2 border-l-2 border-blue-500 pl-3">
                <div>
                  <p className="text-sm">{item.action}</p>
                  <p className="text-xs text-gray-600">{item.time}</p>
                </div>
                <Badge variant="success" className="text-xs">+{item.points}</Badge>
              </div>
            ))}
          </div>
        </div>
