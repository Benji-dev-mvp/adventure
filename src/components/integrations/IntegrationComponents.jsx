...
import { RefreshCw, CheckCircle, XCircle, AlertTriangle, Cloud, MessageSquare, Calendar, Zap, ArrowRight, Settings, Activity, Webhook } from 'lucide-react';
...
export const TwoWaySalesforceSync = () => {
  ...
  const recentSyncs = [
    { object: 'Lead', name: 'John Doe @ Acme Corp', action: 'Updated', status: 'success', time: '30 sec ago' },
    { object: 'Opportunity', name: 'Enterprise Deal - $50K', action: 'Created', status: 'success', time: '1 min ago' },
    { object: 'Activity', name: 'Email sent to Sarah', action: 'Logged', status: 'success', time: '2 min ago' },
    { object: 'Lead', name: 'Mike Johnson @ TechStart', action: 'Updated', status: 'conflict', time: '3 min ago' }
  ];
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-center">
            <p className="text-2xl font-bold text-blue-600">{syncStats.leadsInSync.toLocaleString()}</p>
          ...
          <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg text-center">
            <p className="text-2xl font-bold text-purple-600">{syncStats.activitiesInSync.toLocaleString()}</p>
              ...
              <div key={idx} className="flex items-center justify-between p-3 border rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600">{sync.object} • {sync.action}</p>
                  </div>
                </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {fieldMappings.map((mapping, idx) => (
              <div key={idx} className="flex items-center gap-3 p-2 bg-gray-50 dark:bg-gray-800/50 rounded">
                <Badge variant="secondary" className="text-xs">{mapping.artisan}</Badge>
                <ArrowRight size={14} className="text-gray-400" />
                <Badge variant="secondary" className="text-xs">{mapping.salesforce}</Badge>
...
export const SlackTeamsDeepIntegration = () => {
  const [notifications, setNotifications] = useState([
    { id: 1, channel: '#sales', message: 'Hot lead detected: John Doe @ Acme Corp (Score: 94)', time: '2 min ago', actions: true },
    { id: 2, channel: '#team', message: 'Campaign "Q4 Outbound" needs approval', time: '15 min ago', actions: true },
    { id: 3, channel: '#wins', message: 'Deal closed! $50K from TechStart Inc', time: '1 hour ago', actions: false }
  ]);
              ...
              <div key={notif.id} className="p-3 border rounded-lg">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <Badge variant="secondary" className="text-xs mb-2">{notif.channel}</Badge>
                ...
                {notif.actions && (
                  <div className="flex gap-2 mt-2">
                    <Button size="sm" variant="outline">View in Artisan</Button>
...
export const CalendarAutoScheduler = () => {
  ...
  const [recentSchedules, setRecentSchedules] = useState([
    { prospect: 'John Doe', company: 'Acme Corp', time: 'Tomorrow 10:00 AM PST', status: 'scheduled', type: 'auto' },
    { prospect: 'Sarah Smith', company: 'TechStart', time: 'Dec 29 2:00 PM EST', status: 'scheduled', type: 'auto' },
    { prospect: 'Mike Johnson', company: 'DataCo', time: 'Dec 30 11:00 AM CST', status: 'pending', type: 'manual' }
  ]);
  ...
  return (
    <Card>
      <CardHeader>
          ...
          <div className="flex items-center gap-2">
            ...
            <label className="relative inline-block w-10 h-5">
              ...
              <span className={`absolute cursor-pointer top-0 left-0 right-0 bottom-0 rounded-full transition-all ${
                ...
                <span className={`absolute h-4 w-4 left-0.5 bottom-0.5 bg-white rounded-full transition-all ${
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Automatically schedule meetings from positive replies</p>
      </CardHeader>
      <CardContent>
            ...
            {recentSchedules.map((schedule, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-gray-600">{schedule.company} • {schedule.time}</p>
                </div>
                <div className="flex items-center gap-2">
                  {schedule.type === 'auto' && (
                    <Badge variant="success" className="text-xs">Auto-scheduled</Badge>
                  )}
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {schedulingRules.map((rule, idx) => (
              <label key={idx} className="flex items-center gap-2 p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded">
                <input 
                  type="checkbox" 
        ...
        <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs text-gray-600 mb-1 block">Default Duration</label>
              <select className="w-full text-sm border rounded p-2">
...
export const ZapierActionBuilder = () => {
  const [zapiers, setZapiers] = useState([
    { id: 1, name: 'New Lead → Add to Google Sheets', trigger: 'Lead Created', action: 'Add Row', enabled: true, runs: 847 },
    { id: 2, name: 'Hot Lead → Send Slack Alert', trigger: 'Lead Score > 90', action: 'Send Message', enabled: true, runs: 34 },
    { id: 3, name: 'Deal Won → Update Airtable', trigger: 'Deal Closed Won', action: 'Update Record', enabled: false, runs: 12 }
  ]);

  const availableTriggers = [
    'Lead Created', 'Lead Score Updated', 'Campaign Sent', 'Email Opened', 
    'Email Replied', 'Meeting Booked', 'Deal Closed', 'Hot Lead Detected'
  ];
    ...
    <Card>
      ...
      <CardContent>
            ...
            {zapiers.map((zap) => (
              <div key={zap.id} className="flex items-center justify-between p-3 border rounded-lg">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-600">{zap.trigger} → {zap.action}</p>
                ...
                <div className="flex gap-2">
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      setZapiers(zapiers.map(z => 
                        z.id === zap.id ? {...z, enabled: !z.enabled} : z
                      ));
                    }}
                  >
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {popularActions.map((action, idx) => (
              <div key={idx} className="p-3 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer text-center">
        ...
        <div className="mt-6 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-xs text-gray-600 mb-1 block">When this happens...</label>
              <select className="w-full text-sm border rounded p-2">
            ...
            <div>
              <label className="text-xs text-gray-600 mb-1 block">Do this...</label>
              <select className="w-full text-sm border rounded p-2">
                <option>Select action app</option>
                {popularActions.map((action, idx) => (
                  <option key={idx}>{action.app} - {action.action}</option>
                ))}
              </select>
            </div>
          ...
          {integrations.map((int) => (
            <div key={int.name} className="flex items-center justify-between p-3 border rounded-lg">
              ...
              <div className="text-right">
                ...
                {int.errors > 0 && (
                  <p className="text-xs text-red-600 mt-1">{int.errors} errors</p>
                )}
              </div>
            </div>
          ))}
