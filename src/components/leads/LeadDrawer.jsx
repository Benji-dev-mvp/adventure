...
const MOCK_LEAD = {
  ...
  owner: 'John Smith',
  timeline: [
    { id: 1, type: 'email_opened', title: 'Opened email', description: 'Opened "Q1 Product Update" 3 times', time: '2 hours ago', icon: 'Mail' },
    { id: 2, type: 'page_visit', title: 'Visited pricing page', description: 'Spent 4 min on pricing page', time: '3 hours ago', icon: 'Globe' },
    { id: 3, type: 'email_clicked', title: 'Clicked email link', description: 'Clicked "Schedule Demo" CTA', time: '1 day ago', icon: 'ExternalLink' },
    { id: 4, type: 'score_change', title: 'Score increased', description: 'Score went from 72 to 85', time: '1 day ago', icon: 'TrendingUp' },
    { id: 5, type: 'email_sent', title: 'Email sent', description: 'Sequence: Enterprise Outreach - Step 2', time: '3 days ago', icon: 'Send' },
    { id: 6, type: 'added', title: 'Lead created', description: 'Added from LinkedIn import', time: '2 weeks ago', icon: 'User' },
  ],
  emails: [
    { id: 1, subject: 'Q1 Product Update - New Features', status: 'opened', opens: 3, clicks: 1, sentAt: '2 days ago', campaign: 'Enterprise Outreach' },
    { id: 2, subject: 'Quick question about your sales process', status: 'clicked', opens: 2, clicks: 1, sentAt: '5 days ago', campaign: 'Enterprise Outreach' },
    { id: 3, subject: 'Introduction from Artisan', status: 'opened', opens: 1, clicks: 0, sentAt: '1 week ago', campaign: 'Enterprise Outreach' },
  ],
  aiSummary: {
    intent: 'High',
    ...
    recommendedActions: [
      { action: 'Schedule a demo call', priority: 'high', reason: 'High engagement indicates readiness' },
      { action: 'Send case study', priority: 'medium', reason: 'Match with similar company success story' },
  ...
  playbooks: [
    ...
    { id: 3, name: 'Re-engagement Sequence', fit: 30, status: 'not_started', step: 0, totalSteps: 3 },
  ],
};
...
const ScoreBadge = ({ score }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className={cn("w-10 h-10 rounded-full flex items-center justify-center text-white font-bold", getColor())}>
        {score}
      </div>
      <div>
  ...
  <div className="space-y-6">
    ...
    <div>
      ...
      <div className="space-y-2">
        <a href={`mailto:${lead.email}`} className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 hover:text-accent-500">
          <Mail className="h-4 w-4" />
          {lead.email}
        </a>
        <a href={`tel:${lead.phone}`} className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 hover:text-accent-500">
          <Phone className="h-4 w-4" />
          {lead.phone}
        </a>
        <a href={lead.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 hover:text-accent-500">
      ...
      <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <a href={lead.company.website} target="_blank" rel="noopener noreferrer" className="text-accent-500 hover:underline flex items-center gap-1">
...
const EmailsTab = ({ lead }) => (
  <div className="space-y-3">
    {lead.emails.map((email) => (
      <div key={email.id} className="bg-gray-50 dark:bg-white/5 rounded-xl p-4 hover:bg-gray-100 dark:hover:bg-white/10 transition-colors cursor-pointer">
  ...
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <div className="bg-gradient-to-br from-cyan-500/10 to-blue-500/10 rounded-xl p-4 border border-cyan-500/20">
        ...
        <p className="text-2xl font-bold text-gray-900 dark:text-white">{lead.aiSummary.buyingStage}</p>
      </div>
    </div>
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-4">
        ...
        <p className="text-sm font-medium text-gray-900 dark:text-white">{lead.aiSummary.bestTimeToContact}</p>
      </div>
    </div>
  </div>
);
            ...
            <div className="flex items-center gap-2 mt-2">
              <div className="flex-1 h-2 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                <div 
                  className={cn(
                    "h-full rounded-full",
                    playbook.fit >= 80 ? 'bg-green-500' : playbook.fit >= 50 ? 'bg-amber-500' : 'bg-gray-400'
                  )}
                  style={{ width: `${playbook.fit}%` }}
                />
              </div>
              <span className="text-sm font-medium text-gray-900 dark:text-white">{playbook.fit}%</span>
            </div>
