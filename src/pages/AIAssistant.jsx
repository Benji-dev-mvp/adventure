...
import { getAIMessages, addAIMessage, askAva, applyGuardrail, updateTemplateCTA, scheduleOptimalWindow, getLeadSources, getExampleLead, getSavedPrompts, addSavedPrompt, removeSavedPrompt, getLeads, getSourceIcon, estimateTokens, scoreLeadWithAI, generateEmailWithAI } from '../lib/dataService';
...
const AIAssistant = () => {
  ...
  const handleSend = async (overrideText) => {
    ...
    try {
      ...
      const summary = sourceSummary.find((s) => s.source === contextSource) || { total: 0, verified: 0 };
  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 -z-10 pointer-events-none">
        ...
        {/* Animated Orbs */}
        <div className="absolute top-20 right-1/4 w-96 h-96 bg-gradient-to-br from-cyan-400/20 to-blue-500/20 dark:from-cyan-500/10 dark:to-blue-600/10 rounded-full blur-3xl animate-pulse" 
             style={{ animationDuration: '7s' }} />
        <div className="absolute bottom-20 left-1/4 w-96 h-96 bg-gradient-to-br from-purple-400/20 to-pink-500/20 dark:from-purple-500/10 dark:to-pink-600/10 rounded-full blur-3xl animate-pulse" 
      ...
      <div className="mb-6 relative group">
        ...
        <Card className={cn('relative overflow-hidden backdrop-blur-xl', surfaces.hero)}>
          ...
          <CardContent className="relative p-8">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className={cn('text-sm font-medium', textTones.muted)}>
                        ...
                        <AnimatedCounter end={stats.promptTokens + stats.responseTokens} duration={1000} /> tokens processed
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Source Selector */}
                <div>
                  <label className={cn('text-xs font-bold uppercase tracking-widest', textTones.muted)}>📍 Data Source</label>
                  <div className="mt-3 flex flex-wrap gap-3">
                    {sourceSummary.map((s) => (
                      ...
                      >
                        ...
                        <span className={cn('ml-2 text-xs opacity-75', textTones.muted)}>({s.verified}/{s.total})</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {/* Lead Selector */}
                <div>
                  <label className={cn('text-xs font-bold uppercase tracking-widest', textTones.muted)}>👤 Target Lead</label>
                  <select
            ...
            <div className="mt-6 relative group/analytics">
              ...
              <div className={cn('relative rounded-2xl p-6 transition-all duration-300 border-2', isDark ? 'border-purple-500/30 bg-gradient-to-br from-slate-900/60 via-purple-900/20 to-slate-900/60 backdrop-blur-md hover:border-purple-500/50' : 'border-purple-200 bg-gradient-to-br from-white to-purple-50/30 shadow-xl hover:border-purple-300')}>
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div className="group/stat relative">
                    ...
                    <p className={cn('text-xs uppercase font-black tracking-wider mb-2', textTones.muted)}>
                    ...
                    <p className="text-3xl font-black bg-gradient-to-r from-cyan-500 to-blue-600 bg-clip-text text-transparent">
                      <AnimatedCounter end={stats.promptTokens + stats.responseTokens} duration={1000} />
                    </p>
                    <p className={cn('text-xs mt-2 font-medium', textTones.subtle)}>
                      ↑<AnimatedCounter end={stats.promptTokens} duration={800} /> ↓<AnimatedCounter end={stats.responseTokens} duration={800} />
                    </p>
                  </div>
                  <div className="border-l-2 border-r-2 border-white/10 group/stat relative">
                    ...
                    <p className={cn('text-xs mt-2 font-medium flex items-center justify-center gap-1', textTones.subtle)}>
                  ...
                  <div className="group/stat relative">
                    ...
                    <p className={cn('text-xs mt-2 font-medium text-emerald-600 dark:text-emerald-400 flex items-center justify-center gap-1')}>
                      ⭐ Excellent
                    </p>
                  </div>
                </div>
            ...
            <Card className={cn('relative h-full flex flex-col', surfaces.panel)}>
              <CardContent className="flex-1 flex flex-col p-6">
                ...
                {messages.map((message, index) => (
                  <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in-50 slide-in-from-bottom-4 duration-500`}>
                    <div className={`max-w-[85%] ${message.role === 'user' ? 'order-2' : 'order-1'}`}>
                      {message.role === 'assistant' && (
                        <div className="flex items-center gap-3 mb-3 pl-2">
                          ...
                          <span className="text-sm font-black bg-gradient-to-r from-cyan-600 to-purple-600 dark:from-cyan-400 dark:to-purple-400 bg-clip-text text-transparent">Ava AI</span>
                        </div>
                      )}
                      ...
                      {message.role === 'assistant' && (
                        <div className="mt-3 flex flex-col gap-2">
                          {message.metrics && (
                            <div className={cn('text-xs flex items-center gap-3', textTones.muted)}>
                              <span>📊 {message.metrics.promptTokens + message.metrics.responseTokens} tokens</span>
                          ...
                          <div className="flex flex-wrap gap-2">
                            <Button
                              ...
                              onClick={async () => {
                                try { await navigator.clipboard.writeText(message.content); toast.info('Response copied'); } catch {}
                              }}
                            >
                              📋 Copy
                            </Button>
                            <Button
                              ...
                              onClick={() => {
                                const title = (message.content || '').slice(0, 40) || 'Saved Response';
                ...
                <div className="border-t border-white/30 pt-6 space-y-4 bg-gradient-to-t from-white/10 to-transparent">
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    <div className="group/builder">
                      <label className={cn('text-xs font-bold uppercase tracking-wider', textTones.muted)}>🎭 Persona</label>
                      <input 
                        className={cn(
                    ...
                    <div className="group/builder">
                      <label className={cn('text-xs font-bold uppercase tracking-wider', textTones.muted)}>🎯 Goal</label>
                      <input 
                        className={cn(
                          'w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-transparent transition-all',
                          isDark
                    ...
                    <div className="group/builder">
                      <label className={cn('text-xs font-bold uppercase tracking-wider', textTones.muted)}>⚙️ Constraints</label>
                      <input 
                        className={cn(
                          'w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-transparent transition-all',
                          isDark
                  ...
                  <div className="flex items-end gap-4">
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          try { await navigator.clipboard.writeText(input); toast.info('Prompt copied'); } catch {}
                        }}
                      >
                        📋 Copy
                      </Button>
        ...
        <div className="space-y-4">
          ...
          <div className="relative group">
            ...
            <Card className={cn('relative hover:shadow-3xl transition-all duration-300', surfaces.panel)}>
              <CardContent className="p-6">
                <div className="flex items-center gap-3 mb-5">
                  <div className="p-2.5 bg-gradient-to-br from-accent-500/30 to-primary-500/30 rounded-xl border border-accent-500/20">
                    <Sparkles size={20} className={isDark ? 'text-accent-300' : 'text-accent-600'} />
                  </div>
            ...
            <Card className={cn('relative hover:shadow-3xl transition-all duration-300', surfaces.panel)}>
              <CardContent className="p-6">
                ...
                <div className="space-y-5">
                  <div>
                    <label className={cn('block text-xs font-bold uppercase tracking-wider mb-3', textTones.muted)}>
                      🎭 Tone
                    </label>
                    <div className="flex flex-wrap gap-2">
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-3 mb-5">
                  <div className="p-2.5 bg-gradient-to-br from-yellow-500/30 to-orange-500/30 rounded-xl border border-yellow-500/20">
                    <Lightbulb size={20} className={isDark ? 'text-yellow-300' : 'text-amber-600'} />
                  </div>
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className={cn(
                            ...
                            <Icon size={16} className={isDark ? 'text-yellow-300' : 'text-amber-700'} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={cn('text-sm font-bold', textTones.heading)}>{template.title}</p>
                            <p className={cn('text-xs mt-0.5', textTones.muted)}>{template.description}</p>
                          </div>
                        </div>
                      </button>
                    );
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-3 mb-5">
                  <div className="p-2.5 bg-gradient-to-br from-green-500/30 to-emerald-500/30 rounded-xl border border-green-500/20">
                    <Lightbulb size={20} className={isDark ? 'text-green-300' : 'text-emerald-700'} />
                  </div>
                ...
                <div className="space-y-2.5 max-h-48 overflow-y-auto">
                  {savedPrompts.map((p) => (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <p className={cn('text-sm font-semibold truncate', textTones.heading)}>{p.title}</p>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <Button 
                          variant="outline" 
                          size="sm" 
                          onClick={() => { setSavedPrompts(removeSavedPrompt(p.id)); toast.info('Deleted'); }} 
                          className={cn(
                            'text-xs',
                            isDark
                  ...
                  {savedPrompts.length === 0 && (
                    <p className={cn('text-xs text-center py-4', textTones.muted)}>No saved prompts yet</p>
                  )}
                </div>
              </CardContent>
            </Card>
                  ...
                  {sourceSummary.map((s) => (
                    <div
                      ...
                      className={cn(
                        ...
                        isDark ? 'border-white/20 bg-white/10 hover:bg-white/15' : 'border-gray-200 bg-white hover:bg-slate-50'
                      )}
                    >
                      <span className={cn('font-semibold', textTones.heading)}>{getSourceIcon(s.source)} {s.source}</span>
                      <span className={cn('text-xs font-medium', textTones.muted)}>✓ {s.verified}/{s.total}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-3 mb-5">
                  <div className="p-2.5 bg-gradient-to-br from-pink-500/30 to-red-500/30 rounded-xl border border-pink-500/20">
                    <Sparkles size={20} className={cn(isDark ? 'text-pink-300' : 'text-rose-700', 'animate-pulse')} />
                  </div>
                ...
                <div className="space-y-3">
                  <div className={cn('group/metric flex items-center justify-between p-4 rounded-xl border-2 transition-all hover:scale-[1.02] relative overflow-hidden', isDark ? 'border-pink-500/30 bg-gradient-to-r from-pink-500/10 to-rose-500/10 hover:border-pink-500/50' : 'border-pink-200 bg-gradient-to-r from-pink-50 to-rose-50 hover:border-pink-300')}>
                    ...
                    <div>
                      <span className={cn('text-xs font-bold uppercase tracking-wider', textTones.muted)}>
                    ...
                    <TrendingUp className={cn(isDark ? 'text-pink-400' : 'text-pink-600', 'animate-pulse')} size={24} />
                  </div>
                  
                  <div className={cn('group/metric flex items-center justify-between p-4 rounded-xl border-2 transition-all hover:scale-[1.02] relative overflow-hidden', isDark ? 'border-green-500/30 bg-gradient-to-r from-green-500/10 to-emerald-500/10 hover:border-green-500/50' : 'border-green-200 bg-gradient-to-r from-green-50 to-emerald-50 hover:border-green-300')}>
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-black bg-gradient-to-r from-green-500 to-emerald-600 bg-clip-text text-transparent mt-1">
                        <AnimatedCounter end={Math.floor(liveMetrics.timeSaved)} duration={2000} /> hrs
                      </p>
                    </div>
                    <Zap className={cn(isDark ? 'text-green-400' : 'text-emerald-600', 'animate-pulse')} size={24} />
                  </div>
                  
                  <div className={cn('group/metric flex items-center justify-between p-4 rounded-xl border-2 transition-all hover:scale-[1.02] relative overflow-hidden', isDark ? 'border-purple-500/30 bg-gradient-to-r from-purple-500/10 to-indigo-500/10 hover:border-purple-500/50' : 'border-purple-200 bg-gradient-to-r from-purple-50 to-indigo-50 hover:border-purple-300')}>
                    ...
                    <TrendingUp className={cn(isDark ? 'text-purple-400' : 'text-purple-600', 'animate-pulse')} size={24} />
                  </div>
                </div>

                <div className={cn('mt-4 p-4 rounded-xl border-2 bg-gradient-to-r relative overflow-hidden', isDark ? 'border-cyan-500/30 from-cyan-500/10 to-purple-500/10 text-gray-200' : 'border-cyan-200 from-cyan-50 to-purple-50 text-slate-700')}>
                  ...
                  <p className="text-xs leading-relaxed flex items-start gap-2">
                    <Lightbulb size={16} className="text-cyan-500 flex-shrink-0 mt-0.5 animate-pulse" />
