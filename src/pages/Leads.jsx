...
import { updateLeadStatus, exportLeads, addLeadActivity, getLeads, getActiveCRM } from '../lib/dataService';
...
const Leads = () => {
  ...
  return (
    ...
    >
      <div className="grid lg:grid-cols-3 gap-4">
          ...
          <Card>
            <CardContent className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-sm text-gray-600">1202 total leads • intent enriched and scored</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button className="gap-2" onClick={() => setIsContactOpen(true)}>View Profile</Button>
                </div>
              </div>
            ...
            <CardContent className="p-0">
              <div className="divide-y">
                {filteredLeads.length === 0 && (
                  <div className="p-6 text-center text-sm text-gray-500">No leads match your filters.</div>
                )}
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div className="relative">
                          ...
                          {isSelected && <span className="absolute -right-1 -bottom-1 w-4 h-4 rounded-full bg-accent-500 border-2 border-white" />}
                        </div>
                        <div className="min-w-0">
                          <p className="font-semibold text-gray-900 truncate">{lead.name}</p>
                          <p className="text-sm text-gray-600 truncate">{lead.title} • {lead.company}</p>
                          <p className="text-xs text-gray-500 truncate">{lead.email}</p>
                          {lead.source && (
                            <div className="mt-1">
                              <Badge variant="default" className="bg-white border border-gray-200 text-xs">
                                Source: {lead.source}
                              </Badge>
                            </div>
                          )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <Star size={16} className={lead.status === 'hot' ? 'text-yellow-500' : 'text-gray-600'} />
                          </button>
                        </div>
                      </div>
                    </button>
        ...
        <div className="space-y-4">
          <Card className="shadow-lg border-accent-100">
            <CardContent className="space-y-4">
              {!selectedLead && (
                <div className="text-center text-sm text-gray-500 py-6">Select a lead to view details.</div>
              )}
                ...
                <>
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{selectedLead.title} at {selectedLead.company}</p>
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          <Badge variant="score" className={`bg-gradient-to-r ${getScoreColor(selectedLead.score)}`}>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="p-3 rounded-xl bg-gray-50 border border-gray-100">
                      ...
                      <a href={`mailto:${selectedLead.email}`} className="text-accent-600 font-medium flex items-center gap-1">
                        {selectedLead.email}
                    ...
                    <div className="p-3 rounded-xl bg-gray-50 border border-gray-100">
                      ...
                      <span className="text-gray-800 font-medium">{selectedLead.enrichment.companySize}</span>
                    </div>
                    <div className="p-3 rounded-xl bg-gray-50 border border-gray-100">
                      <p className="text-xs text-gray-500 mb-1">Revenue</p>
                      <span className="text-gray-800 font-medium">{selectedLead.enrichment.revenue}</span>
                    </div>
                    <div className="p-3 rounded-xl bg-gray-50 border border-gray-100">
                  ...
                  <div className="mt-3 p-3 rounded-xl bg-white border border-gray-200">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-sm text-gray-600">
                        Source: <span className="font-medium text-gray-900">{selectedLead.source || activeCRM || 'Unknown'}</span>
                      </div>
                    </div>
                    {!selectedLead.verified && (
                      <div className="mt-2 text-xs text-gray-500">
                        Connect a trusted source to verify this lead. <a href="/integrations" className="text-accent-600 hover:underline">View integrations</a>.
                      </div>
                    )}
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-3">
                      <label className="flex items-center gap-2 text-sm">
                        <input
                          ...
                          onChange={(e) => setLeads((prev) => prev.map((l) => l.id === selectedLead.id ? { ...l, verified: e.target.checked } : l))}
                        />
                        Mark as verified
                      </label>
                      <div>
                        <input
                          ...
                          onChange={(e) => setLeads((prev) => prev.map((l) => l.id === selectedLead.id ? { ...l, source: e.target.value } : l))}
                      ...
                      <div className="col-span-2 flex justify-end">
                        <Button
                          ...
                          className="gap-2"
                          onClick={() => {
                            const updated = updateLeadVerification(selectedLead.id, { verified: selectedLead.verified, source: selectedLead.source });
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedLead.enrichment.techStack.map((tech) => (
                        <Badge key={tech} variant="default" className="bg-white border border-gray-200">
                          {tech}
                        </Badge>
                      ))}
                    </div>
                      ...
                      {selectedLead.activity.map((activity, index) => (
                        <div key={index} className="flex gap-3">
                          <div className="w-9 h-9 bg-accent-50 rounded-lg flex items-center justify-center">
                            {activity.type === 'email_opened' && <Mail size={16} className="text-accent-600" />}
                            {activity.type === 'email_sent' && <Mail size={16} className="text-gray-600" />}
                            {activity.type === 'email_replied' && <Mail size={16} className="text-green-600" />}
                            {activity.type === 'link_clicked' && <TrendingUp size={16} className="text-blue-600" />}
                            {activity.type === 'meeting_booked' && <Calendar size={16} className="text-purple-600" />}
                          </div>
                          <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button variant="outline" className="gap-2" onClick={() => handleEngageLead(selectedLead.id)}>
                      <Phone size={16} /> Call
                    </Button>
                    <Button
                      variant="outline"
                      className="gap-2"
                      onClick={() => handleStatusChange(selectedLead.id, selectedLead.status === 'hot' ? 'warm' : 'hot')}
                    >
      ...
      <Modal isOpen={isContactOpen && !!selectedLead} onClose={() => setIsContactOpen(false)} size="lg">
        {selectedLead && (
          <>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <a href={`mailto:${selectedLead.email}`} className="text-accent-600 hover:underline">
                        {selectedLead.email}
                      </a>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-gray-50 rounded-lg">
                      <p className="text-xs text-gray-500 mb-1">Company Size</p>
                      <p className="font-medium text-gray-900">{selectedLead.enrichment.companySize}</p>
                    </div>
                    <div className="p-3 bg-gray-50 rounded-lg">
                      <p className="text-xs text-gray-500 mb-1">Revenue</p>
